...
myFadeInactiveHook = fadeInactiveLogHook' 0.9
...
myScratchPads = [ NS "mixer"    spawnMixer  findMixer  manageMixer
  ...
  where
    ...
    manageMixer = customFloating $ W.RationalRect l t w h
      where
    		l = 0.2 -- distance from left edge of 20%
    		t = 0.2 -- distance from top edge of 20%
    		w = 0.6	-- 60% width
    		h = 0.4	-- 40% height

    spawnTerm  = myScratchTerminal ++ " -T scratchpad -b"
    ...
    spawnRanger  = myScratchTerminal ++ " -T FileManager -b -e ranger"
    findRanger   = title             =? "FileManager"
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
  ...
  -- Custom key bindings
  --
  -- Summon / dismiss the scratch pad.
  [ ((modMask, xK_u),
     namedScratchpadAction myScratchPads "terminal")

  -- Summon / dismiss the pulse mixer scratch pad.
  , ((modMask, xK_m),
     namedScratchpadAction myScratchPads "mixer")

  -- Summon / dismiss the ranger scratch pad.
  , ((modMask, xK_f),
     namedScratchpadAction myScratchPads "ranger")
...
main = do
  ...
  xmonad $ defaults {
      ...
      , startupHook = do
            setWMName "LG3D"
            spawn "xcompmgr -c -C -t-5 -l-5 -r4.2 -o.55"
            spawn "feh --bg-scale ~/Downloads/3840x2160.jpg"
  }
